...
class PaygopsTokens(models.TransientModel):
    ...
    def get_country(self, sale):
        return sale.company_id.country_id.code.lower()
    ...
    def last_token_generated(self, device_serial, transaction_id, payment_id, partner_id, amount, salesperson, loan_id, phone_number, phone_number_partner, sale_order):
        ...
        URL = API_GATEWAY_URL + "/api/v1/" + self.get_country(sale_order) + "/" + device_serial + "/tokens"
        ...
        if resp.status_code == 200 or resp.status_code == 201:
            if 'tokens' in response:
                if 'error_message' in response["tokens"]:
                    ...
                    token = self.env['credit.token'].create({'code': token_code, 'token_id': token_id, 'duration': duration, 'token_type': token_type, 'credit_end_date': credit_end_date, 'generated_date': format_date(generated_date),
